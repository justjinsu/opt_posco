...
Avoids import issues by running directly.
"""
...
# Now import
import io as posco_io
import model as posco_model
import export as posco_export
import sanity as posco_sanity
...
def run_scenario(data_file, scenario_name, output_dir='outputs'):
    ...
    try:
        # Load parameters
        logger.info(f"Loading parameters from {data_file}...")
        ...
        if 'carbon_price' in params:
            logger.info(f"Carbon prices (key years):")
        ...
        logger.info(f"Exporting time series to {series_path}...")
        posco_export.export_time_series(solution, params, str(series_path))
        logger.info(f"✓ Results exported")

        # Create summary
        summary_path = output_path / f"detailed_results_{scenario_name}.csv"
        logger.info(f"Creating detailed summary...")
        # You can add more detailed export here if needed
